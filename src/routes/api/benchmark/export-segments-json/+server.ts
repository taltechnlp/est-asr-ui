...
function parseSegmentsFromCorrectedText(correctedText: string): ParsedSegment[] {
	...
	if (segments.length > 0) {
		...
		console.log(
			`DEBUG: No segments found. Input preview (first 500 chars):`,
	...
	try {
		...
		if (werCorrections.length === 0) {
			...
			if (analysisSegments.length > 0) {
				console.log(
					`[JSON Export] Sample segment indices:`,
					analysisSegments.slice(0, 5).map((s) => s.segmentIndex)
				);
				console.log(
					`[JSON Export] Sample segment timing:`,
					analysisSegments.slice(0, 2).map((s) => ({
		...
		if (werCorrections.length > 0) {
			...
			console.log(
				`Processing ${completedCorrections.length} completed WER corrections with segment boundary recovery`
			);
			...
			for (const correction of completedCorrections) {
				...
				console.log(
					`Processing WER correction block ${correction.blockIndex} with ${blockSegmentIndices.length} segments`
				);
				...
				if (parsedSegments.length > 0) {
					...
					console.log(
						`Mapping ${parsedSegments.length} parsed segments to ${blockSegmentIndices.length} block segments`
					);
					...
					for (let i = 0; i < parsedSegments.length && i < blockSegmentIndices.length; i++) {
						...
						const asrSegment = speakerSegmentsFromASR.find(
							(asr) => asr.index === actualSegmentIndex
						);
						if (asrSegment) {
							segments.push({
							...
							console.log(
								`Mapped parsed segment ${i} to ASR segment ${actualSegmentIndex} (${asrSegment.start}s-${asrSegment.end}s)`
							);
						} else {
					...
					if (parsedSegments.length !== blockSegmentIndices.length) {
						console.log(
							`WARNING: Parsed segments (${parsedSegments.length}) != block segments (${blockSegmentIndices.length})`
						);
					}
				} else {
					// Fallback: if parsing failed, use original segments for this block
					console.log(
						`Segment parsing failed for block ${correction.blockIndex}, using original text`
					);
					for (const segmentIndex of blockSegmentIndices) {
						...
						if (asrSegment) {
							...
							console.log(
								`Added fallback segment ${segmentIndex}: ${asrSegment.start}s-${asrSegment.end}s`
							);
						}
					}
				}
			...
			for (const asrSegment of speakerSegmentsFromASR) {
				if (!processedSegmentIndices.has(asrSegment.index)) {
					...
					console.log(
						`Added non-corrected segment ${asrSegment.index}: ${asrSegment.start}s-${asrSegment.end}s`
					);
				}
			}
			...
			speakerSegmentsFromASR.forEach((asrSegment, index) => {
				...
				const correspondingAnalysisSegment = analysisSegments.find(
					(as) => as.segmentIndex === index
				);
		...
		if (segments.length === 0) {
			...
			console.error(
				`WER corrections: ${werCorrections.length}, ASR segments: ${speakerSegmentsFromASR.length}, Analysis segments: ${analysisSegments.length}`
			);
		}
