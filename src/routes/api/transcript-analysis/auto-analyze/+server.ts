...
export const POST: RequestHandler = async ({ request, locals }) => {
  try {
    ...
    try {
      ...
      let summary;
      if (existingSummary) {
        await logger.logGeneral('info', 'Using existing summary', { summaryId: existingSummary.id });
        ...
        await logger.logGeneral('info', 'Summary generated successfully', { summaryId: summary.id });
      }
      ...
      await logger.logGeneral('info', 'Extracted segments for analysis', { segmentCount: segments.length });
      ...
      if (transcriptPath && file.id) {
        const logger = getAgentFileLogger(transcriptPath, file.id);
        await logger.logGeneral('error', 'Auto-analysis failed', { error: error instanceof Error ? error.message : String(error) });
      }
      return json({ 
