		...
		if (node.type.name === 'speaker') {
			...
			node.content.forEach((child) => {
				if (child.type.name === 'wordNode') {
					...
					// Track timing
					if (
						child.attrs.start !== undefined &&
						(firstTime === undefined || child.attrs.start < firstTime)
					) {
						firstTime = child.attrs.start;
					}
					if (
						child.attrs.end !== undefined &&
						(lastTime === undefined || child.attrs.end > lastTime)
					) {
...
): Map<number, PositionAwareSegment> {
