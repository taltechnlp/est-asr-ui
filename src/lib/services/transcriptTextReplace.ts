...
function convertStringToProseMirrorPositions(
	...
	doc.descendants((node, pos) => {
		...
		if (node.isText && node.text) {
			...
			if (fromPos === null && stringFrom >= nodeStringStart && stringFrom < nodeStringEnd) {
				...
				console.log(
					`Found from position: string ${stringFrom} -> ProseMirror ${fromPos} (node ${nodeStringStart}-${nodeStringEnd})`
				);
			}
			...
			if (toPos === null && stringTo > nodeStringStart && stringTo <= nodeStringEnd) {
				...
				console.log(
					`Found to position: string ${stringTo} -> ProseMirror ${toPos} (node ${nodeStringStart}-${nodeStringEnd})`
				);
			}
	...
	if (fromPos !== null && toPos !== null) {
		console.log(
			`Successfully converted string positions ${stringFrom}-${stringTo} to ProseMirror ${fromPos}-${toPos}`
		);
...
function findTextUsingProseMirrorSearch(
	...
	while (searchIndex < fullTextLower.length) {
		...
		console.log(
			`Found exact text match at position ${matchIndex}: "${fullText.substring(matchIndex, matchIndex + searchText.length)}"`
		);
		...
		if (actualMatch.toLowerCase() === searchLower) {
			...
			const proseMirrorPositions = convertStringToProseMirrorPositions(
				doc,
				matchIndex,
			...
			if (proseMirrorPositions) {
				...
				console.log(
					`Successfully created exact match: ProseMirror ${from}-${to} = "${actualMatch}"`
				);
			} else {
				console.warn(
					`Failed to convert string positions ${matchIndex}-${matchIndex + searchText.length} to ProseMirror positions`
				);
			}
		}
	...
	if (matches.length === 0) {
		...
		const normalizedSearchLower = caseSensitive
			? normalizedSearchText
			: normalizedSearchText.toLowerCase();
		const normalizedFullTextLower = caseSensitive
			? normalizedFullText
			: normalizedFullText.toLowerCase();

		console.log('Normalized search text:', JSON.stringify(normalizedSearchText));
		console.log(
			'Normalized document text (first 200 chars):',
			JSON.stringify(normalizedFullText.substring(0, 200))
		);
		...
		while (searchIndex < normalizedFullTextLower.length) {
			...
			const matchedNormalizedText = normalizedFullText.substring(
				matchIndex,
			...
			while ((regexMatch = regex.exec(fullText)) !== null) {
				...
				console.log(
					`Regex match found at string positions: ${stringFrom}-${stringTo} = "${actualText}"`
				);
				...
				if (proseMirrorPositions) {
					...
					console.warn(
						`Failed to convert regex match positions ${stringFrom}-${stringTo} to ProseMirror positions`
					);
				}
			}
...
function findCrossNodeMatches(
	...
	doc.descendants((node, pos) => {
		if (node.isText) {
			console.log(
				`  Text node: pos=${pos}, text="${node.text}", length=${node.text.length}, nodeSize=${node.nodeSize}`
			);
	...
	doc.descendants((node, pos) => {
		...
		if (node.isText && node.text) {
			...
			console.log(
				`Text node at pos ${pos}-${pos + textLength}: "${node.text}" (textLength: ${textLength}, nodeSize: ${node.nodeSize})`
			);
		}
	});
	...
	while (searchIndex < fullTextLower.length) {
		...
		console.log(
			`Found match at fullText position ${matchIndex}: "${fullText.substring(matchIndex, matchIndex + searchText.length)}"`
		);
		...
		if (wholeWord) {
			...
			const afterChar =
				matchIndex + searchLower.length < fullTextLower.length
		...
		for (let i = 0; i < textNodes.length; i++) {
			...
			console.log(
				`Node ${i}: pos ${nodeStart}-${nodeEnd} ("${textNode.text}") ProseMirror ${textNode.from}-${textNode.to}`
			);
			...
			if (matchIndex >= nodeStart && matchIndex < nodeEnd && !startNode) {
				...
				console.log(
					`Match starts in node ${i} at offset ${offsetInNode}, ProseMirror pos ${matchFrom}`
				);
			}
			...
			if (matchEnd > nodeStart && matchEnd <= nodeEnd && !endNode) {
				...
				console.log(
					`Match ends in node ${i} at offset ${offsetInNode}, ProseMirror pos ${matchTo}`
				);
			}
		...
		console.log(
			`Position mapping result: startNode=${!!startNode}, endNode=${!!endNode}, matchFrom=${matchFrom}, matchTo=${matchTo}`
		);
		...
		if (startNode && endNode && matchFrom !== -1 && matchTo !== -1) {
			...
			console.log(
				`Successfully created match: from=${matchFrom}, to=${matchTo}, text="${matchedText}"`
			);
			...
			console.log(
				`  startNode=${!!startNode} (${startNode ? `pos ${startNode.from}-${startNode.to}` : 'none'})`
			);
			console.log(
				`  endNode=${!!endNode} (${endNode ? `pos ${endNode.from}-${endNode.to}` : 'none'})`
			);
			console.log(`  matchFrom=${matchFrom}, matchTo=${matchTo}`);
			console.log(
				`  searchText length=${searchText.length}, match position=${matchIndex}-${matchIndex + searchText.length}`
			);
		}
	...
	for (let i = 0; i <= wordNodes.length - searchWords.length; i++) {
		...
		if (matchedWords.length === searchWords.length) {
			...
			if (avgDistance <= maxAvgDistance) {
				console.log(
					`Fuzzy match found at word index ${i} with avg distance ${avgDistance.toFixed(2)}`
				);
...
function buildWordMap(doc: ProseMirrorNode, segmentId?: string) {
	...
	doc.descendants((node, pos) => {
		...
		if (node.isText && node.text) {
			...
			if (wordMark) {
				console.log(
					`  Word node: "${node.text}" at ${pos}-${pos + node.text.length}, marks: word(${wordMark.attrs.start}-${wordMark.attrs.end})`
				);
			} else {
				console.log(
					`  Text node (no word mark): "${node.text}" at ${pos}-${pos + node.text.length}`
				);
			}
		}
	});
...
function findTextUsingWordMap(
	...
	console.log(
		'Continuous text built from words:',
		JSON.stringify(continuousText.substring(0, 200))
	);
	...
	while (searchIndex < textLower.length) {
		...
		for (const pos of positionMap) {
			if (startWordIndex === -1 && matchIndex >= pos.textStart && matchIndex < pos.textEnd) {
				...
				console.log(
					`  Match starts in word ${startWordIndex}: "${wordNodes[startWordIndex].text}" at offset ${startOffset}`
				);
			}
			...
			if (matchEnd > pos.textStart && matchEnd <= pos.textEnd) {
				...
				console.log(
					`  Match ends in word ${endWordIndex}: "${wordNodes[endWordIndex].text}" at offset ${endOffset}`
				);
				break;
			}
		}
		...
		if (startWordIndex !== -1 && endWordIndex !== -1) {
			...
			console.log(
				`  Failed to map match to word nodes: startWord=${startWordIndex}, endWord=${endWordIndex}`
			);
		}
...
export function findTextPositions(
		...
		if (node.isText && node.text) {
			...
			while (searchIndex < nodeText.length) {
				...
				if (wholeWord) {
					...
					const afterChar =
						matchIndex + normalizedSearch.length < nodeText.length
				...
				const originalMatchEnd = findOriginalPosition(
					node.text,
					normalizedNodeText,
		...
		if (docTextForSearch.includes(normalizedSearch)) {
			...
			if (simplifiedMatches.length > 0) {
				...
				if (crossNodeMatches.length > 0) {
					...
					console.log(
						'Cross-node matching failed - text may be in non-text nodes or spans complex structure'
					);
				}
			}
		} else {
			...
			console.log(
				'Full document text (first 500 chars):',
				JSON.stringify(normalizedDocText.substring(0, 500))
			);
			...
			if (foundWords.length > 0) {
				...
				console.log(
					'Missing words:',
					words.filter((word) => !docTextForSearch.includes(word.toLowerCase()))
				);
	...
	if (matches.length === 0) {
		...
		console.log('  - Normalized search text:', JSON.stringify(normalizedSearchText));
		console.log(
			'  - Document text sample (first 500 chars):',
			JSON.stringify(allDocumentText.substring(0, 500))
		);
		console.log(
			'  - Search methods tried: word-aware, individual nodes, cross-node, ProseMirror search, fuzzy'
		);
	}
 ...
 */
function handleSpeakerChange(editor: Editor, suggestion: ImprovementSuggestion): ReplacementResult {
