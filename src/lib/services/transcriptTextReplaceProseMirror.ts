	...
	for (const speakerBlock of speakerBlocks) {
		...
		for (const { node, pos: localPos } of textNodes) {
			...
			if (text) {
				...
				console.log(
					`  Text node: "${text}" at absolute pos ${absolutePos}, local pos ${localPos}, has word mark: ${!!wordMark}`
				);
			}
		}
			...
			if (matchIndex !== -1) {
				...
				for (let i = 0; i < wordSequence.length; i++) {
					...
					if (startWordIdx === -1 && matchIndex >= charCount && matchIndex < wordEnd) {
						...
						console.log(
							`    Match starts in word ${i}: "${wordSequence[i].text}" at offset ${startOffset}`
						);
					}
					...
					if (matchEnd > charCount && matchEnd <= wordEnd) {
						...
						console.log(
							`    Match ends in word ${i}: "${wordSequence[i].text}" at offset ${endOffset}`
						);
						break;
					}
		...
		for (let i = 0; i < wordSequence.length; i++) {
			...
			for (let wordIdx = 0; wordIdx < searchWords.length; wordIdx++) {
				...
				for (
					let j = currentIndex;
					j < Math.min(currentIndex + maxLookAhead, wordSequence.length);
					j++
				) {
					...
					if (wordsMatch(nodeWord, searchWord, caseSensitive)) {
						...
						if (searchDistance > 0) {
							...
							debugInfo.push(
								`  Word ${wordIdx} "${searchWord}": Found at distance ${searchDistance}, skipped: [${skippedText}]`
							);
							console.log(
								`    ⚠️ Had to skip ${searchDistance} nodes to match "${searchWord}": [${skippedText}]`
							);
						} else {
						...
						found = true;
						console.log(
							`    Matched word "${searchWord}" with node "${nodeText}" (normalized: "${nodeWord}") at pos ${wordSequence[j].pos}`
						);
						break;
					}
					...
					if (j + 1 < wordSequence.length) {
						...
						if (wordsMatch(combined, searchWord, caseSensitive)) {
							...
							found = true;
							debugInfo.push(
								`  Word ${wordIdx} "${searchWord}": Found as combined nodes at index ${j}`
							);
							console.log(
								`    Matched word "${searchWord}" with combined nodes at pos ${wordSequence[j].pos}`
							);
							break;
						}
					}
				...
				if (!found) {
					...
					debugInfo.push(
						`  Word ${wordIdx} "${searchWord}": NOT FOUND within ${maxLookAhead} nodes from index ${currentIndex}`
					);

					// Log what we saw instead
					const nearbyNodes = wordSequence.slice(
						currentIndex,
						Math.min(currentIndex + 5, wordSequence.length)
					);
					const nearbyText = nearbyNodes.map((n) => `"${normalizeWord(n.text)}"`).join(', ');
					console.log(
						`    ❌ Could not find "${searchWord}" near index ${currentIndex}. Next nodes: [${nearbyText}]`
					);
					break;
				}
			}
			...
			if (matchedWords === searchWords.length && matchSequence.length > 0) {
				...
				const hasLargeGaps = debugInfo.some(
					(info) =>
						info.includes('distance') && parseInt(info.match(/distance (\d+)/)?.[1] || '0') > 2
				);
...
export function findAndReplaceText(
	...
	if (matches.length === 0) {
		...
		const foundWords = words.filter((w) => allText.toLowerCase().includes(w.toLowerCase()));

		if (foundWords.length > 0) {
			errorMsg += ` Found ${foundWords.length}/${words.length} words: [${foundWords.join(', ')}].`;
			const missingWords = words.filter((w) => !allText.toLowerCase().includes(w.toLowerCase()));
