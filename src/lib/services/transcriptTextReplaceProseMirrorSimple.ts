...
export function findAndReplaceTextSimple(
	...
	if (matches.length === 0) {
		...
		const foundWords = words.filter((w) => fullText.toLowerCase().includes(w.toLowerCase()));

		if (foundWords.length > 0) {
			errorMsg += ` Found ${foundWords.length}/${words.length} words: [${foundWords.join(', ')}].`;
			const missingWords = words.filter((w) => !fullText.toLowerCase().includes(w.toLowerCase()));
