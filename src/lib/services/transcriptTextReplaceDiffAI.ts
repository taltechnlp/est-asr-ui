...
export function createDiffAtPosition(
	...
	try {
		...
		editor.chain().focus().deleteRange({ from, to }).insertContentAt(from, diffNode.toJSON()).run();
		...
		if (!found) {
			...
			doc.nodesBetween(0, doc.content.size, (node, pos) => {
				if (node.type.name === 'wordNode') {
					...
					positionMap.push({
						start: startIndex,
						end: startIndex + (node.text?.length || 0),
						docPos: pos
					});
				}
			});
			...
			if (searchIndex !== -1) {
				...
				const startEntry = positionMap.find(
					(entry) => entry.start <= searchIndex && searchIndex < entry.end
				);
				// Find the last position entry that contains our end
				const endEntry = positionMap.find(
					(entry) => entry.start < searchEnd && searchEnd <= entry.end
				);
				...
				if (startEntry && endEntry) {
					...
					diffResult = createDiffAtPosition(editor, from, to, searchText, suggestedText, options);
					found = true;
				}
			}
		}
 ...
 */
export function convertTextToWordNodes(editor: Editor, from: number, to: number): boolean {
	try {
		...
		editor.chain().focus().deleteRange({ from, to }).insertContentAt(from, contentWithSpaces).run();
...
function tokenizeSearchText(searchText: string, caseSensitive = false): string[] {
	return splitToWordsUnicode(searchText)
		.map((w) => normalizeWordToken(w, caseSensitive))
		.filter(Boolean);
}
...
function findFuzzyConsecutiveSequence(
	...
	for (let i = 0; i + windowLen <= tokens.length; i++) {
		...
		if (avg >= thresholds.avgSimilarity) {
			...
			if (
				!best ||
				cand.avg > best.avg ||
				(cand.avg === best.avg && cand.to - cand.from > best.to - best.from) ||
				(cand.avg === best.avg &&
					cand.to - cand.from === best.to - best.from &&
					cand.from < best.from)
			) {
				best = cand;
	...
	return best
		? { from: best.from, to: best.to, stats: { avg: best.avg, mismatches: best.mismatches } }
		: null;
}

function resolveSegmentBoundsById(
	editor: Editor,
	segmentId: string
...
export function findAndCreateDiffFuzzy(
	...
	try {
		...
		const thresholds = {
			...
			maxMismatches:
				options.thresholds?.maxMismatches ??
				Math.max(1, Math.floor((splitToWordsUnicode(searchText).length || 1) * 0.2))
		};
		...
		const tokensPrimary = buildWordNodeTokenStream(doc, bounds ?? undefined, {
			includePlainTextWords: false,
			caseSensitive
		});
		...
		if (!match && bounds) {
			const tokensSecondary = buildWordNodeTokenStream(doc, bounds, {
				includePlainTextWords: true,
				caseSensitive
			});
		...
		if (!match && !bounds) {
			const tokensAll = buildWordNodeTokenStream(doc, undefined, {
				includePlainTextWords: false,
				caseSensitive
			});
		...
		if (match) {
			...
			const ok =
				slice.toLocaleLowerCase('et').includes(first) &&
				slice.toLocaleLowerCase('et').includes(last);
			if (!ok) {
