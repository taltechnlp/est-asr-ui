...
const combineWords = (words: Array<Word>) => (acc: any[], word: { word_with_punctuation: string; start: number; end: number }) => {
...
const mapTurns = (turn: NewTurn, sps: NewSpeakers, speakers: Array<Speaker>, words: Array<Word>, alternatives: string = "") => {
    let name = 'S1';
    if (sps && turn.speaker) {
        ...
        bestHypothesis.sections.forEach(section => {
            if (section.type === 'speech' && section.turns) {
                section.turns.forEach((turn) => {
                    ...
                    const speakerNode = mapTurns(turn, bestHypothesis.speakers, speakers, words, alternativesJson);
