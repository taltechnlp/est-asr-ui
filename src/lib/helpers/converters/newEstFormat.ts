import type { TranscriptionResult, BestHypothesis, NewTurn, NewSpeaker as NewSpeakers } from '$lib/helpers/api.d';
...
const combineWords = (acc: string, word: { word_with_punctuation: string; start: number; end: number }) => {
    const id = uuidv4().substring(36 - 12);
    words.push({ start: word.start, end: word.end, id });
    return (
        acc + `<span start="${word.start}" end="${word.end}" id="${id}">${word.word_with_punctuation} </span>`
    );
};
...
const mapTurns = (turn: NewTurn, sps: NewSpeakers, alternatives: string = "") => {
    ...
    const alternativesAttr = alternatives ? ` alternatives="${alternatives.replace(/"/g, '&quot;')}"` : '';
...
export const fromNewEstFormat = (transcriptionResult: TranscriptionResult) => {
    ...
    if (transcriptionResult && transcriptionResult.best_hypothesis && transcriptionResult.best_hypothesis.sections) {
                ...
                if (section.type === 'speech' && section.turns) {
                    const res = acc.concat(
                        section.turns.map((turn, turnIndex) => {
                            ...
                            const alternativesJson = matched && matched.alternatives.length > 0 
 ...
 */
export const getAlternativesFromSegment = (segmentElement: any): Array<{ rank: number; text: string; avg_logprob: number }> => {
    try {
