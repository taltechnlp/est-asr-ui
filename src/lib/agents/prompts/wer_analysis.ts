...
- Synthesize information across all N-best hypotheses
- Generate corrections that combine best elements from multiple alternatives
...
// Cache-optimized prompt structure - static instructions that can be cached
export const WER_STATIC_INSTRUCTIONS = `You are analyzing a transcript for Word Error Rate (WER) improvement using an evidence-based agentic approach. Your primary goal is to identify and correct only segments where the ASR system was acoustically uncertain, following the "do no harm" principle.
...
// Dynamic content template that changes per request
export const WER_DYNAMIC_CONTENT_TEMPLATE = `Response language: {responseLanguage}
...
export interface WERPrompts {
	...
	STATIC_INSTRUCTIONS: string;
	DYNAMIC_CONTENT_TEMPLATE: string;
	CLARIFICATION_PROMPT: string;
}

export const WER_PROMPTS: WERPrompts = {
	AGENTIC_ANALYSIS_PROMPT: WER_AGENTIC_ANALYSIS_PROMPT,
	STATIC_INSTRUCTIONS: WER_STATIC_INSTRUCTIONS,
	DYNAMIC_CONTENT_TEMPLATE: WER_DYNAMIC_CONTENT_TEMPLATE,
