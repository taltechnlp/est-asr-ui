			...
			if (existingSummary) {
				...
				if (existingSummary.uiLanguage !== uiLanguage || !existingSummary.displaySummary) {
					...
					if (uiLanguage !== 'en') {
						...
						const displayKeyTopics = await this.translateKeyTopics(
							existingSummary.keyTopics,
							uiLanguage
						);
...
Please provide the corrected JSON response with proper formatting.`;
				...
				try {
					...
					if (!parseResult.success) {
						throw new Error(
							`Failed to parse summary JSON even after correction: ${parseResult.error}`
						);
					}
			...
			if (missingFields.length > 0) {
				...
				const fieldRecoveryResult = await this.recoverMissingFields(
					result,
					missingFields,
					transcript
				);
	...
	private async generateDisplaySummary(
		transcript: string,
		uiLanguage: SupportedLanguage
	): Promise<string> {
		try {
			const languageName = getLanguageName(uiLanguage);
			const prompt = DISPLAY_SUMMARY_PROMPT.replace('{transcript}', transcript).replace(
				'{language}',
				languageName
			);
			...
			throw new Error(
				`Failed to generate display summary: ${error instanceof Error ? error.message : 'Unknown error'}`
			);
		}
	}

	private async translateKeyTopics(
		topics: string[],
		uiLanguage: SupportedLanguage
	): Promise<string[]> {
		try {
			const languageName = getLanguageName(uiLanguage);
			const prompt = TRANSLATE_KEY_TOPICS_PROMPT.replace('{topics}', topics.join('\n')).replace(
				/{language}/g,
				languageName
			);
	...
	async deleteSummary(fileId: string): Promise<void> {
		await prisma.transcriptSummary
			.delete({
				where: { fileId }
			})
			.catch(() => {
...
CRITICAL: Return ONLY the complete JSON object. No explanations or other text.`;
			...
			try {
				...
				if (
					recoveryParseResult.success &&
					typeof recoveryParseResult.data === 'object' &&
					!Array.isArray(recoveryParseResult.data)
				) {
					console.log('✅ Format recovery successful');
...
CRITICAL: Return ONLY the complete JSON object. No explanations or other text.`;
	...
	private async recoverMissingFields(
		...
		const missingFieldPrompts = missingFields
			.map((field) => `- ${field}: ${fieldDescriptions[field] || 'Required field'}`)
			.join('\n');
...
CRITICAL: Return ONLY the complete JSON object. No explanations or other text.`;
	...
	private createFallbackSummary(result: any, transcript: string): SummaryResult {
		...
		if (Array.isArray(result)) {
			...
			summary =
				result.summary || result.text || result.description || 'Transcript analysis completed.';
		...
		if (!summary || summary.length < 20) {
			summary =
				`This transcript contains ${transcript.length} characters of content with ${speakerCount} speaker(s). ` +
		...
		console.log(
			`📋 Fallback summary created: ${summary.length} chars, ${keyTopics.length} topics, ${speakerCount} speakers`
		);
