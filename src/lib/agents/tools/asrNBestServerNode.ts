...
export class ASRNBestServerNodeTool {
	...
	async getAlternativeTranscriptions(
		input: z.infer<typeof ASRNBestSchema>
	): Promise<ASRNBestResult> {
		...
		if (startTime < 0) {
			await logger?.logToolExecution('ASR-NBest', 'Invalid startTime parameter', {
				startTime,
				error: 'must be >= 0'
			});
		...
		if (endTime <= startTime) {
			await logger?.logToolExecution('ASR-NBest', 'Invalid time range', {
				startTime,
				endTime,
				error: 'endTime must be > startTime'
			});
			throw new Error(
		...
		if (duration < 0.1) {
			await logger?.logToolExecution('ASR-NBest', 'Warning: very short segment duration', {
				duration,
				warning: 'ASR may not produce meaningful results'
			});
		}

		if (duration > 120) {
			await logger?.logToolExecution('ASR-NBest', 'Warning: long segment duration', {
				duration,
				warning: 'consider splitting for better accuracy'
			});
		}
		...
		try {
			...
			if (lastCloseBrace === -1) {
				...
				throw new Error(
					`Malformed JSON in curl output: ${stdout.substring(firstBrace, Math.min(firstBrace + 500, stdout.length))}...`
				);
			}
			...
			if (asrResult.service && asrResult.version && asrResult.endpoints) {
				throw new Error(
					`ASR API returned service metadata instead of transcription. Response: ${JSON.stringify(asrResult)}`
				);
			}
			...
			const result = {
				alternatives,
				primaryText:
					asrResult.text || (alternatives.length > 0 ? alternatives[0].text : originalText),
			...
			await logger?.logToolExecution('ASR-NBest', 'Parsed ASR API response structure', {
				...
				primaryText:
					result.primaryText.substring(0, 100) + (result.primaryText.length > 100 ? '...' : '')
			});
			...
			throw new Error(
				`Failed to get ASR alternatives: ${error instanceof Error ? error.message : 'Unknown error'}`
			);
		} finally {
