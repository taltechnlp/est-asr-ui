...
const PositionAwareTipTapSchema = z.object({
  ...
  changeType: z.enum(['text_replacement', 'speaker_change', 'punctuation', 'grammar'])
    .default('text_replacement').describe("Type of change being made"),
...
export class PositionAwareTipTapTool extends TranscriptAnalysisTool {
  ...
  constructor() {
    const bindedApplyChange = (input: PositionAwareTipTapInput) => this.applyPositionBasedChange(input);
...
The tool expects position information relative to segments sent to the LLM.`,
  ...
  private async applyPositionBasedChange(input: PositionAwareTipTapInput): Promise<string> {
    ...
    const { segmentId, startChar, endChar, originalText, suggestedText, changeType, confidence, context } = input;
    ...
    try {
      ...
      if (mappedRange.valid) {
        ...
        if (!isValid) {
          ...
          if (recovered) {
            ...
            return this.fallbackToTextSearch(originalText, suggestedText, changeType, confidence, context);
          }
        }
      } else {
      ...
      const success = this.editor.chain().focus().command(({ tr, state }) => {
        ...
        if (!matches && !fallbackUsed) {
          console.error(`Text mismatch at position: expected "${originalText}", found "${currentText}"`);
          return false;
        }
