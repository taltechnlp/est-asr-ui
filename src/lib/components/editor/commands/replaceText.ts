 ...
 */
export const findAndReplace: Command = (options: FindAndReplaceOptions) => ({ editor, tr, dispatch }) => {
 ...
 */
export const replaceAtPosition: Command = (options: {
  from: number;
  to: number;
  text: string;
 ...
 */
export const highlightMatch: Command = (match: TextMatch) => ({ tr, state, dispatch }) => {
  if (!dispatch) return true;
  ...
  tr.setSelection(state.schema.selection.between(
    state.doc.resolve(match.from),
    state.doc.resolve(match.to)
  ));
 ...
 */
export const updateSpeakerName: Command = (options: {
  segmentId: string;
  newName: string;
}) => ({ tr, state, dispatch }) => {
