	...
	<div class="controls flex justify-between items-center pt-0.5 pb-0.5">
		<div class="flex justify-center items-center">
			...
			<input
				...
				class="ml-1 hidden sm:block bg-base-100"
				onchange={zoomSlider}
			/>
			<button class="btn btn-square btn-sm control-button ml-1" onclick={zoomOut}>
				<Icon data={minus} scale={1} />
			</button>
			<select class="select ml-2" bind:value={rate} onchange={changeSpeed}>
				<option>0.5x</option>
				<option>0.6x</option>
				<option>0.7x</option>
				<option>0.8x</option>
		...
		<div class="flex justify-around items-center w-48 min-w-max">
			<div class="tooltip" data-tip="Shift + Tab">
				<button class="btn btn-square control-button" onclick={seekBackward} aria-label="Seek backward">
					<svg
						viewBox="0 0 24 24"
			...
			<div class="tooltip" data-tip="Alt + Tab">
				<button class="btn btn-square control-button" onclick={seekForward} aria-label="Seek forward">
