...
describe('Position-Based Text Replacement', () => {
			...
			nodesBetween: (from: number, to: number, callback: Function) => {
				...
				const nodes = [
					...
					},
					{
						...
						isText: true,
						nodeSize: 33,
						marks: [{ type: { name: 'word' }, attrs: { start: 0, end: 5 } }]
					}
				];
	...
	describe('Position Mapping', () => {
		...
		it('should validate position content', () => {
			const isValid = validatePositionContent(mockDoc, 0, 5, 'Hello');
	...
	describe('Position Recovery', () => {
		it('should recover position by text context', () => {
			const recovered = PositionRecovery.recoverByTextContext(mockDoc, 'test', 15, 50);
		...
		it('should find nearest valid position', () => {
			const nearest = PositionRecovery.findNearestValidPosition(mockDoc, 50, 'after');
		...
		it('should validate and adjust position range', () => {
			const validated = PositionRecovery.validateRange(mockDoc, -10, 2000);
